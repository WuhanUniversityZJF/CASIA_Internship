...
import bitsandbytes as bnb  # 新增 bitsandbytes 导入
...
def parse_args(args):
    ...
    parser.add_argument("--local-rank", default=1, type=int, help="node rank")
...
def main(args):
    ...
    # 使用 bitsandbytes 8bit 加载模型，自动映射设备，减小显存占用
    ...
    # 加载训练权重（保持原逻辑）
    trained_weight = torch.load("C:\\Users\\Dobot\\Desktop\\Lenna-main\\attn_weight.pt")
    ...
    # 不用手动调用 model.float().cuda()，device_map='auto' 会自动管理设备
    ...
    while True:
        ...
        image_clip = (clip_image_processor.preprocess(image_np, return_tensors="pt")["pixel_values"][0].unsqueeze(0).to(model.device))
